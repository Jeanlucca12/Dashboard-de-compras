...
data_inicio = st.sidebar.date_input("Data inicial", value=data_default - timedelta(days=6))
data_final = st.sidebar.date_input("Data final", value=data_default)
...
# Verifica se existem compras no período selecionado
if not df_compras_filter.empty:

    # Loja com maior quantidade de vendas
    principal_loja = df_compras_filter["loja"].value_counts().idxmax()

    st.markdown(f"## Loja Principal: {principal_loja}")

    col21, col22 = st.columns(2)

    # Valor total vendido pela loja principal
    valor_compras_loja = df_compras_filter.loc[
        df_compras_filter["loja"] == principal_loja,
        "preco"
    ].sum()

    # Quantidade de compras da loja principal
    quantidade_compras_loja = df_compras_filter.loc[
        df_compras_filter["loja"] == principal_loja,
        "preco"
    ].count()

    col21.metric("Valor vendido", f"R$ {valor_compras_loja:.2f}")
    col22.metric("Quantidade de compras", quantidade_compras_loja)
...
col31.metric("Valor das compras no período", valor_compras_vendedor) 
col32.metric("Comissão do vendedor no período", valor_comissao_vendedor) 